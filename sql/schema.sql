...
create type user_role as enum ('admin', 'producao', 'armazem', 'consulta', 'qualidade', 'manutencao');

create table public.perfis (
    id uuid primary key references auth.users(id) on delete cascade,
    nome text not null,
    role user_role not null default 'consulta',
    -- Distingue o criador (acesso total) de um admin de gestão (ex.
    -- Mónica) — este último vê o mesmo admin.html, mas sem Utilizadores
    -- nem Importações, e sem poder gerir acessos extra/etiquetas de
    -- outras pessoas. Continua a ser role='admin' nos dois casos — isto
    -- é só um nível extra por cima, não um perfil novo.
    super_admin boolean not null default false,
    -- Login deixou de ser por email — passa a ser um nome de utilizador
    -- (ex. "f.sena"). O Supabase Auth continua a exigir um "email" por
    -- baixo (auth.users.email), mas esse passa a ser só uma etiqueta
    -- interna de login (ex. "f.sena@login.batardas.interno"), nunca
    -- mostrada nem usada para enviar nada — daí a separação com
    -- email_contacto a seguir.
    nome_utilizador text unique not null,
    -- Email REAL, opcional — só para onde os alertas por email vão. Quem
    -- não tiver (sem email de empresa, ou caixa partilhada) fica sem
    -- alertas por email, mas continua a ver tudo dentro do site.
    email_contacto text,
    data_aniversario date,
    -- Caminho no Storage (bucket "fotos-perfil"), não um URL público —
    -- o bucket é privado, tal como o "nc-anexos".
    foto_caminho text,
    -- Bloqueia o resto do site até a pessoa preencher nome, data de
    -- aniversário e foto, na primeira vez que entra.
...
-- Impede que alguém sem ser admin mude o próprio role/super_admin — o
-- update de perfil (nome, foto, aniversário) fica aberto a cada pessoa
-- editar a sua própria linha, mas role e super_admin são demasiado
-- sensíveis para depender só de RLS ao nível da linha (RLS não
-- restringe COLUNAS, só linhas). Este trigger repõe os valores antigos
-- sempre que quem edita não é admin.
create or replace function public.impedir_auto_promocao()
returns trigger as $$
begin
    if public.meu_role() <> 'admin' then
        new.role := old.role;
...
revoke execute on function public.impedir_auto_promocao() from public;
...
-- ============================================================================
-- MULTI-PERFIL + ETIQUETAS DE RESPONSABILIDADE
-- ============================================================================
-- perfis.role continua a ser o perfil PRINCIPAL — nenhuma regra de
-- segurança já existente muda de comportamento (é essa a razão de ser
-- desta escolha: zero risco no que já funciona). Isto é só ACESSOS
-- EXTRA — ex. a Maria tem role='consulta' mas também precisa de entrar
-- em qualidade.html; fica registado aqui, sem tocar no role principal.
create table public.acessos_extra (
    perfil_id uuid not null references public.perfis(id) on delete cascade,
    role user_role not null,
    criado_em timestamptz not null default now(),
    primary key (perfil_id, role)
);

alter table public.acessos_extra enable row level security;
create policy "leitura de acessos_extra para autenticados" on public.acessos_extra
    for select using (auth.role() = 'authenticated');
create policy "gerir acessos_extra so super_admin" on public.acessos_extra
    for all using (public.sou_super_admin())
    with check (public.sou_super_admin());

-- Devolve true/false se o utilizador autenticado tem acesso a um role,
-- somando o principal + os extra — usar isto em exigirRole() no browser
-- em vez de comparar só perfis.role.
create or replace function public.tenho_acesso(p_role user_role)
returns boolean as $$
    select exists (
        select 1 from public.perfis where id = auth.uid() and role = p_role
        union
        select 1 from public.acessos_extra where perfil_id = auth.uid() and role = p_role
    );
$$ language sql stable security definer set search_path = public;

revoke execute on function public.tenho_acesso(user_role) from public;
grant execute on function public.tenho_acesso(user_role) to authenticated;

-- Etiquetas de responsabilidade — diferentes de um "role" (que dá acesso
-- a um ecrã inteiro): uma etiqueta só decide quem pode ASSUMIR/FECHAR um
-- pedido dentro de uma área (ex. "manutencao"), sem dar acesso a nada
-- por si só. Texto livre em vez de enum — evita alterações ao schema
-- sempre que surgir uma área nova.
create table public.responsabilidades (
    perfil_id uuid not null references public.perfis(id) on delete cascade,
    etiqueta text not null,
    criado_em timestamptz not null default now(),
    primary key (perfil_id, etiqueta)
);

alter table public.responsabilidades enable row level security;
create policy "leitura de responsabilidades para autenticados" on public.responsabilidades
    for select using (auth.role() = 'authenticated');
create policy "gerir responsabilidades so super_admin" on public.responsabilidades
    for all using (public.sou_super_admin())
    with check (public.sou_super_admin());

create or replace function public.tenho_etiqueta(p_etiqueta text)
returns boolean as $$
    select exists (select 1 from public.responsabilidades where perfil_id = auth.uid() and etiqueta = p_etiqueta);
$$ language sql stable security definer set search_path = public;

revoke execute on function public.tenho_etiqueta(text) from public;
grant execute on function public.tenho_etiqueta(text) to authenticated;
...
create type unidade_movimentacao as enum ('un', 'kg', 'l', 'mil', 'pack', 'pack2', 'pack4', 'pack6', 'caixa', 'palete');
...
create type estado_encomenda as enum (
    'Registada', 'Em Produção', 'Em Preparação', 'Pronta', 'Embalada', 'Carregada'
);
...
create table public.encomendas (
    ...
    -- Novo passo (Pronta → Embalada → Carregada): o Armazém regista os
    -- materiais de embalagem gastos (embalagem_consumo, mais abaixo) e
    -- quaisquer quebras nesse embalamento antes de dar como concluído.
    data_conclusao_embalamento timestamptz,
    embalado_por uuid references public.perfis(id),
...
create table public.lotes_producao (
    ...
    data_producao date,
    validade date,
    quantidade_produzida numeric check (quantidade_produzida >= 0),
...
-- Zonas de AUDITORIA (Cozinha, Casa de banho, Receção...) — deliberadamente
-- uma tabela à parte de "localizacoes" (zonas de ARMAZÉM, tipo Câmara
-- Fria/A3): são conceitos diferentes, uma auditoria de higiene não
-- acontece numa zona de stock. Gerir isto fica reservado ao super
-- admin, tal como os outros dados de configuração do site.
create table public.zonas_auditoria (
    id uuid primary key default uuid_generate_v4(),
    nome text unique not null,
    criado_em timestamptz not null default now()
);

alter table public.zonas_auditoria enable row level security;
create policy "leitura de zonas_auditoria para autenticados" on public.zonas_auditoria
    for select using (auth.role() = 'authenticated');
create policy "gerir zonas_auditoria so super_admin" on public.zonas_auditoria
    for all using (public.sou_super_admin())
    with check (public.sou_super_admin());
...
declare
    v_factor numeric;
begin
    -- Un, Kg e L não convertem para nada — são elas próprias a unidade
    -- base do artigo (um artigo medido a Kg não tem "unidades", o Kg É
    -- a unidade). Milheiro e os packs fixos (2/4/6) são multiplicadores
    -- sempre iguais, não dependem do artigo. Pack/Caixa/Palete genéricos
    -- continuam a depender do que configuraste por artigo, porque uma
    -- caixa de ovos e uma caixa de farinha não têm o mesmo tamanho.
    if p_unidade in ('un', 'kg', 'l') then
        return p_quantidade;
    end if;

    v_factor := case p_unidade
        when 'mil' then 1000
        when 'pack2' then 2
        when 'pack4' then 4
        when 'pack6' then 6
...
create table public.plano_producao (
    ...
    -- Para itens de PA (só o admin cria o lote): o Nuno liga aqui o lote
    -- de PA já existente que satisfaz este item do plano, em vez de o
    -- produzir — fica nulo até ele atribuir.
    lote_atribuido_id uuid references public.lotes_producao(lote_id),
    criado_por uuid references public.perfis(id) default auth.uid(),
    criado_em timestamptz not null default now()
);
...
create policy "actualizar plano_producao admin ou producao" on public.plano_producao
...
-- Alargada a "qualquer autenticado" (não só admin) — várias funcionalidades
-- (escolher responsáveis por uma não-conformidade, atribuir acessos extra)
-- precisam de listar nome+perfil de toda a gente, e nome/perfil não é
-- informação sensível (passwords/emails vivem em auth.users, inacessível
-- por aqui). Antes disto, o Qualidade só se via a si próprio na lista de
-- responsáveis possíveis — não conseguia atribuir a mais ninguém.
create policy "leitura de perfis para autenticados" on public.perfis
    for select using (auth.role() = 'authenticated');
-- Cada pessoa edita a própria linha (nome, foto, aniversário, email de
-- contacto, marcar perfil completo) — role/super_admin ficam protegidos
-- pelo trigger impedir_auto_promocao(), não por esta política (RLS não
-- restringe colunas). Admin continua a poder editar qualquer linha.
create policy "actualizar o proprio perfil ou admin" on public.perfis
    for update using (id = auth.uid() or public.meu_role() = 'admin');
...
create table public.alertas (
    ...
    corpo text,
    destinatario_role text not null check (destinatario_role in ('admin', 'producao', 'armazem', 'consulta', 'qualidade', 'manutencao')),
    link_secao text,
    link_id uuid,
...
-- As três políticas usam tenho_acesso() em vez de comparar só o perfil
-- principal — sem isto, alguém com acesso extra a um módulo (ex.
-- Consulta com acesso extra a Qualidade) nunca via os alertas desse
-- módulo no sino, mesmo conseguindo entrar no ecrã.
create policy "leitura de alertas pelo destinatario" on public.alertas
    for select using (public.tenho_acesso(destinatario_role::user_role));
create policy "marcar alertas como lidos pelo destinatario" on public.alertas
    for update using (public.tenho_acesso(destinatario_role::user_role));
create policy "eliminar alertas pelo destinatario" on public.alertas
    for delete using (public.tenho_acesso(destinatario_role::user_role));
-- Insert restrito a quem tem mesmo uma razão legítima para criar um
-- alerta manualmente (hoje só a sugestão de preço do Consulta/admin) —
-- todos os outros alertas do site vêm de triggers, que correm como
-- security definer e por isso nunca precisam desta política para nada.
create policy "criar alertas admin ou consulta" on public.alertas
    for insert with check (public.meu_role() in ('admin', 'consulta'));
...
-- Lotes de produção: leitura geral. Escrita distinta por tipo de artigo —
-- PA (produto acabado, com o código oficial de lote) fica reservado ao
-- admin; Intermédio (massa, recheio, pão ralado) continua livre para o
-- Nuno, que os produz e atribui-lhes um lote interno sozinho. A
-- ATRIBUIÇÃO de um lote de PA já existente a um item do plano do Nuno
-- (mais abaixo) é uma decisão diferente — essa sim, o Nuno faz sozinho.
alter table public.lotes_producao enable row level security;
create policy "leitura de lotes_producao para autenticados" on public.lotes_producao
    for select using (auth.role() = 'authenticated');
create policy "criar lotes_producao admin ou producao_intermedio" on public.lotes_producao
    for insert with check (
        public.meu_role() = 'admin'
        or (public.meu_role() = 'producao' and exists (
            select 1 from public.artigos where artigo_id = lotes_producao.artigo_id and tipo_produto = 'intermedio'
        ))
    );
create policy "actualizar lotes_producao admin ou producao_intermedio" on public.lotes_producao
    for update using (
        public.meu_role() = 'admin'
        or (public.meu_role() = 'producao' and exists (
            select 1 from public.artigos where artigo_id = lotes_producao.artigo_id and tipo_produto = 'intermedio'
...
create table public.auditorias (
    ...
    zona_id uuid references public.zonas_auditoria(id),
    lote_artigo_id uuid references public.lotes_artigo(lote_artigo_id),
...
-- Eliminar fica limitado a quem criou (ex. enganaste-te agora mesmo) ou
-- ao admin — não é aberto a qualquer Qualidade apagar o trabalho de
-- outro colega, só o próprio ou o super admin corrigem esse tipo de
-- engano.
create policy "eliminar auditorias criador ou admin" on public.auditorias
    for delete using (criado_por = auth.uid() or public.meu_role() = 'admin');
...
-- Substituiu o antigo campo único responsavel_id — uma não-conformidade
-- pode agora ter vários responsáveis ao mesmo tempo (pedido explícito:
-- "às vezes é necessário"). Qualquer um deles pode marcar "Por
-- confirmar" quando a sua parte estiver feita.
create table public.nc_responsaveis (
    ...
    primary key (nao_conformidade_id, perfil_id)
);

alter table public.nc_responsaveis enable row level security;
create policy "leitura de nc_responsaveis para autenticados" on public.nc_responsaveis
    for select using (auth.role() = 'authenticated');
create policy "gerir nc_responsaveis qualidade ou admin" on public.nc_responsaveis
    for all using (public.meu_role() in ('qualidade', 'admin'))
    with check (public.meu_role() in ('qualidade', 'admin'));
...
-- Update restrito a quem tem mesmo que ver com esta não-conformidade —
-- Qualidade/admin (sempre) ou um dos responsáveis atribuídos a ELA
-- especificamente (não "qualquer autenticado", que deixava qualquer
-- pessoa editar não-conformidades de áreas com que não tinha nada a
-- ver). O responsável continua a poder marcar "Por confirmar" sozinho;
-- confirmar como "Resolvida" fica só controlado pela app, mostrando esse
-- botão só a Qualidade/admin.
create policy "actualizar nao_conformidades responsaveis ou qualidade" on public.nao_conformidades
    for update using (
        public.meu_role() in ('qualidade', 'admin')
        or exists (
            select 1 from public.nc_responsaveis
            where nao_conformidade_id = nao_conformidades.id and perfil_id = auth.uid()
        )
    );

-- Alerta a CADA responsável (pode ser mais do que um) quando lhes
-- atribuis uma não-conformidade nova — vai para todos os do MESMO perfil
-- de cada um (ver nota sobre alertas serem por perfil partilhado, não
-- por pessoa). Corre à parte da criação da não-conformidade em si,
-- porque os responsáveis só ficam associados depois (nc_responsaveis é
-- inserida a seguir a nao_conformidades, do lado da app).
...
begin
    select descricao into v_descricao from public.nao_conformidades where id = new.nao_conformidade_id;
    select role into v_role_responsavel from public.perfis where id = new.perfil_id;
    if v_role_responsavel is null then
        return new;
    end if;
    insert into public.alertas (tipo, titulo, corpo, destinatario_role, link_secao, link_id)
    values ('nao_conformidade', 'Nova não-conformidade atribuída', left(coalesce(v_descricao, ''), 140), v_role_responsavel, 'qualidade', new.nao_conformidade_id);
    return new;
end;
$$ language plpgsql security definer set search_path = public;
...
after insert on public.nc_responsaveis
...
-- ============================================================================
create type estado_inventario as enum ('Em curso', 'Concluído pelo armazém', 'Revisto');

create table public.inventarios (
    id uuid primary key default uuid_generate_v4(),
    localizacao_id uuid references public.localizacoes(localizacao_id),
    estado estado_inventario not null default 'Em curso',
    pedido_por uuid references public.perfis(id) default auth.uid(),
    pedido_em timestamptz not null default now(),
    concluido_em timestamptz,
    revisto_em timestamptz,
    revisto_por uuid references public.perfis(id)
);

-- Um item por (artigo, lote) esperado nessa zona no momento em que o
-- inventário foi pedido — quantidade_esperada é a "fotografia" do que o
-- sistema achava que lá estava; quantidade_contada é o que o Fernando
-- encontrou fisicamente. lote_artigo_id fica nulo quando é um lote que o
-- Fernando encontrou e o sistema não sabia que existia (acrescentado por
-- ele durante a contagem).
create table public.inventario_itens (
    ...
    numero_lote text not null,
    lote_artigo_id uuid references public.lotes_artigo(lote_artigo_id),
    quantidade_esperada numeric not null default 0 check (quantidade_esperada >= 0),
    quantidade_contada numeric check (quantidade_contada >= 0),
    contado boolean not null default false
);

create index idx_inventario_itens_inventario on public.inventario_itens(inventario_id);

alter table public.inventarios enable row level security;
create policy "leitura de inventarios para autenticados" on public.inventarios
    for select using (auth.role() = 'authenticated');
create policy "criar inventarios so admin" on public.inventarios
    for insert with check (public.meu_role() = 'admin');
create policy "actualizar inventarios admin ou armazem" on public.inventarios
    for update using (public.meu_role() in ('admin', 'armazem'));

alter table public.inventario_itens enable row level security;
create policy "leitura de inventario_itens para autenticados" on public.inventario_itens
    for select using (auth.role() = 'authenticated');
create policy "gerir inventario_itens admin ou armazem" on public.inventario_itens
    for all using (public.meu_role() in ('admin', 'armazem'))
    with check (public.meu_role() in ('admin', 'armazem'));

-- Ao pedires um inventário, pré-popula os itens a partir do stock actual
-- dessa zona — poupa o Fernando de ter de escrever à mão os artigos que
-- já lá deviam estar.
create or replace function public.popular_itens_inventario()
returns trigger as $$
begin
    insert into public.inventario_itens (inventario_id, artigo_id, numero_lote, lote_artigo_id, quantidade_esperada)
    select new.id, la.artigo_id, la.numero_lote, la.lote_artigo_id, la.quantidade_atual
    from public.lotes_artigo la
    where la.localizacao_id = new.localizacao_id;
    return new;
end;
$$ language plpgsql security definer set search_path = public;

revoke execute on function public.popular_itens_inventario() from public;

create trigger trg_popular_itens_inventario
after insert on public.inventarios
for each row execute function public.popular_itens_inventario();

-- Alerta ao armazém quando lhe pedes um inventário, e ao admin quando o
-- armazém o dá como concluído — mesmo padrão dos outros alertas do site.
create or replace function public.alertar_inventario_pedido()
returns trigger as $$
declare
    v_nome_zona text;
begin
    select nome into v_nome_zona from public.localizacoes where localizacao_id = new.localizacao_id;
    insert into public.alertas (tipo, titulo, corpo, destinatario_role, link_secao, link_id)
    values ('inventario_pedido', 'Inventário pedido', coalesce(v_nome_zona, 'zona'), 'armazem', 'inventario', new.id);
    return new;
end;
$$ language plpgsql security definer set search_path = public;

revoke execute on function public.alertar_inventario_pedido() from public;

create trigger trg_alertar_inventario_pedido
after insert on public.inventarios
for each row execute function public.alertar_inventario_pedido();

create or replace function public.alertar_inventario_concluido()
returns trigger as $$
declare
    v_nome_zona text;
begin
    if new.estado = 'Concluído pelo armazém' and old.estado is distinct from 'Concluído pelo armazém' then
        select nome into v_nome_zona from public.localizacoes where localizacao_id = new.localizacao_id;
        insert into public.alertas (tipo, titulo, corpo, destinatario_role, link_secao, link_id)
        values ('inventario_concluido', 'Inventário concluído — por rever', coalesce(v_nome_zona, 'zona'), 'admin', 'stock', new.id);
    end if;
    return new;
end;
$$ language plpgsql security definer set search_path = public;

revoke execute on function public.alertar_inventario_concluido() from public;

create trigger trg_alertar_inventario_concluido
after update of estado on public.inventarios
for each row execute function public.alertar_inventario_concluido();

-- ============================================================================
-- ANEXOS DE NÃO-CONFORMIDADES — provas e soluções (fotos/documentos)
-- ============================================================================
-- Os ficheiros em si vivem no Supabase Storage (bucket "nc-anexos", criado
-- à mão no painel — ver README), fora do schema.sql. Esta tabela só guarda
-- o caminho para lá chegar, não o ficheiro. O bucket é privado — a app
-- gera um link temporário (signed URL) sempre que precisas de ver/abrir
-- um anexo, nunca um link público permanente.
create type tipo_anexo_nc as enum ('prova', 'solucao');

create table public.nc_anexos (
    id uuid primary key default uuid_generate_v4(),
    nao_conformidade_id uuid not null references public.nao_conformidades(id) on delete cascade,
    tipo tipo_anexo_nc not null,
    caminho_ficheiro text not null,
    nome_original text,
    tipo_mime text,
    carregado_por uuid references public.perfis(id) default auth.uid(),
    carregado_em timestamptz not null default now()
);

create index idx_nc_anexos_nc on public.nc_anexos(nao_conformidade_id);

alter table public.nc_anexos enable row level security;
create policy "leitura de nc_anexos para autenticados" on public.nc_anexos
    for select using (auth.role() = 'authenticated');
-- Insert restrito a quem tem mesmo que ver com esta não-conformidade —
-- mesma lógica do update, para não deixar qualquer pessoa anexar
-- ficheiros a não-conformidades de áreas alheias.
create policy "criar nc_anexos responsaveis ou qualidade" on public.nc_anexos
    for insert with check (
        public.meu_role() in ('qualidade', 'admin')
        or exists (
            select 1 from public.nc_responsaveis
            where nao_conformidade_id = nc_anexos.nao_conformidade_id and perfil_id = auth.uid()
        )
    );

-- ============================================================================
-- MANUTENÇÃO — equipamentos, preventiva, e pedidos de reparação/obra
-- ============================================================================
-- "manutencao" é um role como Qualidade — dá entrada num ecrã próprio.
-- QUALQUER perfil pode REPORTAR uma avaria (a tabela pedidos_manutencao
-- tem insert aberto a todos); só quem tiver acesso ao ecrã de Manutenção
-- E a etiqueta de responsabilidade "manutencao" é que pode assumir/
-- fechar um pedido em concreto — são as duas peças (role + etiqueta) a
-- trabalhar juntas, como ficou combinado.
create type urgencia_manutencao as enum ('Baixa', 'Média', 'Alta', 'Crítica');
create type estado_manutencao as enum ('Pendente', 'Aguarda peças', 'Em curso', 'Concluído');
...
create table public.pedidos_manutencao (
    id uuid primary key default uuid_generate_v4(),
    equipamento_id uuid references public.equipamentos(id),
    descricao text not null,
    urgencia urgencia_manutencao not null default 'Média',
    estado estado_manutencao not null default 'Pendente',
    -- Se o conserto precisou de alguém de fora — sem custos por agora
    -- (decisão explícita), só o registo de que foi externo.
    tecnico_externo boolean not null default false,
    nome_tecnico text,
    -- Liga opcionalmente a uma quebra de produção que este pedido
    -- resolveu — dá ao motivo da quebra um "porquê" mais rico do que só
    -- texto livre (ex. "forno avariado" → aponta para a reparação real).
    lote_producao_id uuid references public.lotes_producao(lote_id),
    reportado_por uuid references public.perfis(id) default auth.uid(),
    criado_em timestamptz not null default now(),
    assumido_por uuid references public.perfis(id),
    concluido_em timestamptz
);
...
create table public.manutencao_preventiva (
    ...
    ultima_realizada_em date,
    observacoes text,
    criado_em timestamptz not null default now()
);

alter table public.equipamentos enable row level security;
create policy "leitura de equipamentos para autenticados" on public.equipamentos
    for select using (auth.role() = 'authenticated');
create policy "gerir equipamentos manutencao ou admin" on public.equipamentos
    for all using (public.meu_role() = 'admin' or public.tenho_acesso('manutencao'))
    with check (public.meu_role() = 'admin' or public.tenho_acesso('manutencao'));

alter table public.pedidos_manutencao enable row level security;
create policy "leitura de pedidos_manutencao para autenticados" on public.pedidos_manutencao
    for select using (auth.role() = 'authenticated');
create policy "reportar pedidos_manutencao autenticados" on public.pedidos_manutencao
    for insert with check (auth.role() = 'authenticated');
-- Assumir/fechar exige a etiqueta de responsabilidade, não só acesso ao
-- ecrã — várias pessoas podem ter acesso extra a Manutenção só para
-- verem o estado, sem poderem tocar nos pedidos.
create policy "actualizar pedidos_manutencao com etiqueta" on public.pedidos_manutencao
    for update using (public.meu_role() = 'admin' or (public.tenho_acesso('manutencao') and public.tenho_etiqueta('manutencao')));

alter table public.manutencao_preventiva enable row level security;
create policy "leitura de manutencao_preventiva para autenticados" on public.manutencao_preventiva
    for select using (auth.role() = 'authenticated');
create policy "gerir manutencao_preventiva com etiqueta" on public.manutencao_preventiva
    for all using (public.meu_role() = 'admin' or (public.tenho_acesso('manutencao') and public.tenho_etiqueta('manutencao')))
    with check (public.meu_role() = 'admin' or (public.tenho_acesso('manutencao') and public.tenho_etiqueta('manutencao')));

-- Alerta a quem tem acesso a Manutenção sempre que alguém reporta uma
-- avaria — mesmo padrão dos outros alertas do site.
create or replace function public.alertar_novo_pedido_manutencao()
returns trigger as $$
begin
    insert into public.alertas (tipo, titulo, corpo, destinatario_role, link_secao, link_id)
    values ('pedido_manutencao', 'Novo pedido de manutenção', left(new.descricao, 140), 'manutencao', 'pedidos', new.id);
    return new;
end;
$$ language plpgsql security definer set search_path = public;

revoke execute on function public.alertar_novo_pedido_manutencao() from public;

create trigger trg_alertar_pedido_manutencao
after insert on public.pedidos_manutencao
for each row execute function public.alertar_novo_pedido_manutencao();

-- ============================================================================
-- EMBALAMENTO — novo passo Pronta → Embalada → Carregada, com consumo de
-- materiais de embalagem e quebras que sobem o custo final do produto
-- acabado (reaproveita o mesmo mecanismo já usado para quebras de
-- produção — ver calcular_custo_lote).
-- ============================================================================
create table public.embalamento_itens (
    id uuid primary key default uuid_generate_v4(),
    encomenda_id uuid not null references public.encomendas(encomenda_id) on delete cascade,
    artigo_id uuid not null references public.artigos(artigo_id),
    quantidade numeric not null check (quantidade > 0),
...
-- Uma quebra no embalamento (ex. produto partiu-se a fechar a caixa)
-- reduz a quantidade_produzida do lote de origem — o mesmo lote continua
-- a ter o mesmo custo_total (gastaste a mesma matéria-prima), mas
-- divide-se agora por menos unidades vendáveis, subindo o custo médio.
create table public.embalamento_quebras (
    id uuid primary key default uuid_generate_v4(),
    encomenda_id uuid not null references public.encomendas(encomenda_id) on delete cascade,
    lote_producao_id uuid not null references public.lotes_producao(lote_id),
    quantidade numeric not null check (quantidade > 0),
...
alter table public.embalamento_itens enable row level security;
create policy "leitura de embalamento_itens para autenticados" on public.embalamento_itens
    for select using (auth.role() = 'authenticated');
create policy "criar embalamento_itens admin ou armazem" on public.embalamento_itens
    for insert with check (public.meu_role() in ('admin', 'armazem'));

alter table public.embalamento_quebras enable row level security;
create policy "leitura de embalamento_quebras para autenticados" on public.embalamento_quebras
    for select using (auth.role() = 'authenticated');
create policy "criar embalamento_quebras admin ou armazem" on public.embalamento_quebras
    for insert with check (public.meu_role() in ('admin', 'armazem'));

-- Consome o material de embalagem por FIFO — mesma lógica da genealogia
-- de matéria-prima, só que aqui o Fernando escolhe directamente o quê e
-- quanto gastou, em vez de vir de uma receita.
create or replace function public.consumir_embalagem_fifo()
returns trigger as $$
declare
    v_restante numeric;
    v_lote record;
    v_a_tirar numeric;
begin
    v_restante := new.quantidade;
    for v_lote in
        select lote_artigo_id, quantidade_atual from public.lotes_artigo
        where artigo_id = new.artigo_id and quantidade_atual > 0
        order by criado_em asc
    loop
        exit when v_restante <= 0;
        v_a_tirar := least(v_lote.quantidade_atual, v_restante);
        update public.lotes_artigo set quantidade_atual = quantidade_atual - v_a_tirar where lote_artigo_id = v_lote.lote_artigo_id;
        v_restante := v_restante - v_a_tirar;
    end loop;
    return new;
end;
...
revoke execute on function public.consumir_embalagem_fifo() from public;

create trigger trg_consumir_embalagem_fifo
after insert on public.embalamento_itens
for each row execute function public.consumir_embalagem_fifo();

-- Move a quantidade da quebra de "produzida" para "quebras" no lote de
-- origem — o total fica igual (não volta a consumir matéria-prima), mas
-- o custo médio por unidade sobe, porque passa a dividir por menos.
create or replace function public.aplicar_quebra_embalamento()
returns trigger as $$
begin
    update public.lotes_producao
    set quantidade_produzida = greatest(0, coalesce(quantidade_produzida, 0) - new.quantidade),
        quebras = coalesce(quebras, 0) + new.quantidade
    where lote_id = new.lote_producao_id;
    return new;
end;
$$ language plpgsql security definer set search_path = public;

revoke execute on function public.aplicar_quebra_embalamento() from public;

create trigger trg_aplicar_quebra_embalamento
after insert on public.embalamento_quebras
for each row execute function public.aplicar_quebra_embalamento();

-- ============================================================================
-- PLANO DE EMBALAMENTO — o que embalar num dia, a partir de lotes de PA já
-- produzidos. Espelha o plano_producao, mas aponta directamente para o
-- lote (não para o artigo) — o Armazém escolhe QUAL lote específico vai
-- embalar naquele dia, não só "quanto".
-- ============================================================================
create table public.plano_embalamento (
    id uuid primary key default uuid_generate_v4(),
    lote_producao_id uuid not null references public.lotes_producao(lote_id),
    quantidade numeric not null check (quantidade > 0),
    data_planeada date not null,
...
create index idx_plano_embalamento_data on public.plano_embalamento(data_planeada);

alter table public.plano_embalamento enable row level security;
create policy "leitura de plano_embalamento para autenticados" on public.plano_embalamento
    for select using (auth.role() = 'authenticated');
create policy "gerir plano_embalamento admin ou armazem" on public.plano_embalamento
    for insert with check (public.meu_role() in ('admin', 'armazem'));
create policy "eliminar plano_embalamento admin ou armazem" on public.plano_embalamento
    for delete using (public.meu_role() in ('admin', 'armazem'));
