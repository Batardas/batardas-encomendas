...
create policy "gerir acessos_extra so admin" on public.acessos_extra
    for all using (public.meu_role() = 'admin')
...
create policy "leitura de responsabilidades para autenticados" on public.responsabilidades
    for select using (auth.role() = 'authenticated');
create policy "gerir responsabilidades so admin" on public.responsabilidades
    for all using (public.meu_role() = 'admin')
    with check (public.meu_role() = 'admin');
