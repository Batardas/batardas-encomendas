...
// email"). Trata quatro eventos diferentes, cada um com destinatários próprios:
...
//   4. Inventário revisto (UPDATE, estado passa a Revisto) -> admin, com
//      o resumo completo das diferenças já em tabela HTML
//
...
async function obterEmailsPorRole(role: string): Promise<string[]> {
    // Lê email_contacto (o real, opcional) — nunca o email de login
    // (auth.users.email), que desde o Bloco 5 é só uma etiqueta interna
    // (ex. "f.sena@login.batardas.interno") e nunca chega a ninguém.
    // Quem não tiver email_contacto preenchido fica sem alertas por
    // email, mas continua a ver tudo dentro do site.
    const { data: perfis } = await adminClient.from("perfis").select("email_contacto").eq("role", role);
    return (perfis ?? []).map((p) => p.email_contacto).filter((e): e is string => !!e);
}
...
async function montarMensagemInventarioRevisto(inventario: Record<string, unknown>): Promise<{ assunto: string; corpo: string }> {
    let nomeZona = "—";
    if (inventario.localizacao_id) {
        const { data } = await adminClient.from("localizacoes").select("nome").eq("localizacao_id", inventario.localizacao_id as string).maybeSingle();
        if (data) nomeZona = data.nome;
    }

    const { data: itens } = await adminClient
        .from("inventario_itens")
        .select("numero_lote, quantidade_esperada, quantidade_contada, artigos(designacao)")
        .eq("inventario_id", inventario.id as string);

    const linhas = (itens ?? []).map((item) => {
        const artigo = (item.artigos as { designacao?: string } | null)?.designacao ?? "—";
        const diferenca = item.quantidade_contada != null ? Number(item.quantidade_contada) - Number(item.quantidade_esperada) : null;
        const corDiferenca = diferenca && diferenca < 0 ? "color:#bc202e;" : "";
        return `<tr>
            <td style="padding:4px 8px; border-bottom:1px solid #eee;">${artigo}</td>
            <td style="padding:4px 8px; border-bottom:1px solid #eee;">${item.numero_lote}</td>
            <td style="padding:4px 8px; border-bottom:1px solid #eee;">${item.quantidade_esperada}</td>
            <td style="padding:4px 8px; border-bottom:1px solid #eee;">${item.quantidade_contada ?? "—"}</td>
            <td style="padding:4px 8px; border-bottom:1px solid #eee; ${corDiferenca}">${diferenca != null ? (diferenca > 0 ? "+" : "") + diferenca : "—"}</td>
        </tr>`;
    }).join("");

    return {
        assunto: `Inventário revisto — ${nomeZona}`,
        corpo: `
            <p>O inventário da zona <strong>${nomeZona}</strong> foi revisto e o stock já foi ajustado no site.</p>
            <p>Resumo, para carregares no Primavera:</p>
            <table style="border-collapse:collapse; font-size:13px;">
                <thead><tr>
                    <th style="text-align:left; padding:4px 8px; border-bottom:2px solid #ccc;">Artigo</th>
                    <th style="text-align:left; padding:4px 8px; border-bottom:2px solid #ccc;">Lote</th>
                    <th style="text-align:left; padding:4px 8px; border-bottom:2px solid #ccc;">Esperado</th>
                    <th style="text-align:left; padding:4px 8px; border-bottom:2px solid #ccc;">Contado</th>
                    <th style="text-align:left; padding:4px 8px; border-bottom:2px solid #ccc;">Diferença</th>
                </tr></thead>
                <tbody>${linhas}</tbody>
            </table>
        `,
...
serve(async (req) => {
    ...
    } else if (table === "inventarios" && type === "UPDATE" && record.estado === "Revisto" && old_record?.estado !== "Revisto") {
        const mensagem = await montarMensagemInventarioRevisto(record);
        await enviarEmail(EMAILS_ADMIN, mensagem.assunto, mensagem.corpo);
    }
