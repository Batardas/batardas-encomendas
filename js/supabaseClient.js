...
// ecrãs — Nuno, Fernando, Consulta, além do próprio Qualidade) ------------
async function carregarMinhasNcPartilhado(meuId) {
    const cartao = document.getElementById("cartaoMinhasNc");
    if (!cartao) return;
    const { data, error } = await supabaseClient
        .from("nc_responsaveis")
        .select("nao_conformidades(id, descricao, gravidade, prazo, estado)")
        .eq("perfil_id", meuId);
    if (error) { console.error(error); return; }

    const ncs = (data ?? []).map((d) => d.nao_conformidades).filter((nc) => nc && nc.estado !== "Resolvida");
    // Só aparece quando há mesmo algo atribuído — pedido explícito, para
    // não mostrar um cartão vazio à espera de nada em cada perfil.
    if (!ncs.length) { cartao.style.display = "none"; return; }
    ...
    ncs.forEach((nc) => {
        ...
        const acao = nc.estado === "Por confirmar"
        ...
        tr.innerHTML = `
            <td data-rotulo="Descrição">${escaparHtml(nc.descricao)}</td>
            <td data-rotulo="Gravidade">${escaparHtml(nc.gravidade)}</td>
            <td data-rotulo="Prazo">${nc.prazo ? formatarData(nc.prazo) : "—"}</td>
            <td data-rotulo="Estado">${escaparHtml(nc.estado)}</td>
