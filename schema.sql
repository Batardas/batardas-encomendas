...
create type user_role as enum ('admin', 'producao', 'armazem', 'consulta');
...
begin
    ...
    if v_tem_bom and v_todos_com_custo then
        select sum(bc.quantidade_por_unidade * coalesce(new.quantidade_produzida, 0) * ca.custo_unitario)
        into v_custo_total
        from public.bom_componentes bc
        left join public.custos_artigo ca on ca.artigo_id = bc.componente_id
        where bc.produto_id = new.artigo_id;
...
before update of quantidade_produzida on public.lotes_producao
for each row execute function public.calcular_custo_lote();
